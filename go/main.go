...
func (h *handlers) AddAnnouncement(c echo.Context) error {
	...
	if _, err := tx.Exec("INSERT INTO `unread_announcements` (`announcement_id`, `user_id`) SELECT ?, `registrations`.`user_id` FROM `registrations` WHERE `registrations`.`course_id` = ?", req.ID, req.CourseID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
